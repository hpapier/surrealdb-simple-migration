...
pub async fn migrate(db: &Surreal<Any>, migration_dir_path: &str) -> Result<(), Error> {
...
async fn setup_migration_table(db: &Surreal<Any>) -> Result<(), surrealdb::Error> {
    let sql = r#"
    ...
    let _ = db.query(sql).await?.check()?;

    Ok(())
}

async fn run_migration_files(db: &Surreal<Any>, migration_dir_path: &str) -> Result<(), Error> {
    ...
    while let Some(dir_entry) = dir.next_entry().await? {
        let filename = dir_entry
            ...
            .unwrap()
            .to_string()
            .replace((migration_dir_path.to_owned() + "/").as_str(), "");
    ...
    for entry in entries {
        ...
        if migrated {
            let updated_at: DateTime<Utc> = File::metadata(&file).await?.modified()?.into();
            ...
            if updated_at > last_migration.unwrap().created_at {
                ...
                return Err(Error::ForbiddenUpdate(format!(
                    "Forbidden: The migration file '{}' has been updated after the last migration.",
                    entry
                )));
            }
            ...
            if last_migration != None
                && last_migration.unwrap().created_at
                    > DateTime::<Utc>::from(File::metadata(&file).await?.modified()?)
            {
                println!(
                    "[X] The migration file '{}' appears before the last migration file '{}'.",
                    &entry,
                    last_migration.unwrap().filename
                );

                return Err(Error::ForbiddenUpdate(format!(
                    "The migration file '{}' appears before the last migration file '{}'.",
                    &entry,
                    last_migration.unwrap().filename
                )));
            }
        ...
        let position = remaining_migrations
            .iter()
            .position(|migration| *migration.filename == entry);
    ...
    if remaining_migrations.len() > 0 {
        println!(
            "[X] Some migration files are missing - migrations failed: {:?}",
            remaining_migrations
        );
        return Err(Error::ForbiddenRemoval(format!(
            "Some migration files are missing - migrations failed: {:?}",
            remaining_migrations
        )));
    }
...
mod tests {
    ...
    async fn clean_up() {
        ...
        let _ = db
            ...
            .await
            .expect("Failed to delete migrations table.");
    }
    ...
    async fn it_migrates_migration_files() {
        ...
        // Setup database.
        let db = any::connect("ws://0.0.0.0:8000")
            .await
        ...
        let _ = create_dir_all(migration_dir_path)
            .expect("Failed to create directory for migration files.");
        let mut file1 =
            File::create(migration_dir_path.to_owned() + "/001_create_user_table.surql")
                .await
                .unwrap();
        file1
            .write_all(
        ...
        let mut file2 =
            File::create(migration_dir_path.to_owned() + "/002_create_post_table.surql")
                .await
                .unwrap();
        file2
            .write_all(
        ...
        let mut file3 =
            File::create(migration_dir_path.to_owned() + "/003_create_comment_table.surql")
                .await
                .unwrap();
        file3
            .write_all(
        ...
        let mut file4 = File::create(migration_dir_path.to_owned() + "/004_i18n_table.surql")
            .await
            .unwrap();
        file4
            .write_all(
        ...
        let mut file5 =
            File::create(migration_dir_path.to_owned() + "/005_create_likes_table.surql")
                .await
                .unwrap();
        file5
            .write_all(
        ...
        super::migrate(&db, migration_dir_path)
            .await
            .expect("Failed to migrate the files.");
        tokio::fs::remove_file(migration_dir_path.to_owned() + "/001_create_user_table.surql")
            .await
            .unwrap();
